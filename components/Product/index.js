...
export default function ProductContent(props) {
  ...
  const handleReviewSubmit = (e) => {
    e.preventDefault();
    const tokenType = getTokenType();
    if(tokenType==='guest' || tokenType===null){
  ...
  const handleColorSelect = (color,activeIndex) => {
    console.log('Here we have activeIndex',activeIndex)
    setSliderActiveIndex(activeIndex)
      ...
      {product !== null &&
        <div className="card product-details-card position-relative">
          ...
          <div className='' style={{width:'100%',overflow:'hidden'}} >
            <BannerSlider product={product} activeIndex={sliderActiveIndex} handleColorSelect={handleColorSelect} productColors={productColors}/>
          </div>
                    ...
                    <div className="card-body">
                      ...
                      <div className="pdc-sizes">
                        {option.value.map((size, sidx) =>
                          <button
                            onClick={() => handleSizeSelect(size,sidx)}
                            key={sidx}
              ...
              <div className="card-body py-0">
                ...
                <div className="pdc-extra">
                  ...
                  <div className="row">
                    ...
                    <div className="col">
                      {product.stock_status ? <span className='text-success'>In Stock</span> : <span className='text-danger'>Out of Stock</span>}
                    </div>
                  </div>
                  <div className="row">
                    <div className="col">
          ...
          <div className="pdc-add-to-cart">
            <button className='btn btn-default btn-lg' style={{border:"1px solid #ccc",display:"block"}}>
              <span><del>{product.price}</del> {product.discount_price} </span>
            </button>
            <button onClick={() => postAddToCart(product.sku)} className='btn btn-primary btn-lg' style={{display:"block"}}>
              <span>ADD TO CART {addToCartLoader && <Loader type="inline-loader" />} </span>
            </button>
          </div>
