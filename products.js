...
export default function Products(props) {
  ...
  const imgUrl        = process.env.NEXT_PUBLIC_IMAGE_URL || 'https://newness.net/pub/media/catalog/product/' ;

  const [categoryId, setCategoryId] = useState(queryString.catId ? queryString.catId : '73');
    ...
    <div className="product-search-page">
      ...
      <div className="sticky-filter-header" >
        ...
        <SlidingPane
          ...
          from="bottom"
          width="100%"
          title={<div className='custom-pane-header'
          onClick={()=>handleClearSearch()}
          ><Image src="/logo.png" alt="Newness" width={696 * 0.2} height={142 * 0.2} /><span className='link'>Clear All</span></div>}
        ...
        >
          <div className="filterpane">
            ...
            <div className="pfactions">
                <button className='btn btn-secondary' onClick={()=>HandleOpenCloseFilter(false)} >Cancel</button>
        ...
        <div className="psproduct-header d-flex align-items-center justify-content-between">
          ...
          <div className="cart-icon">
            ...
            <span className='cart-with-count position-relative'>
              <span className='badge rounded-1 badge-primary bg-primary text-white position-absolute'>{appData.cart.count}</span>
              <AiOutlineShoppingCart />
            </span>
            </Link>
          </div>
        ...
        <div className="product-short-bar d-flex align-items-center justify-content-between">
          ...
          <span className='pfilter-items' onClick={() => HandleOpenCloseFilter(true)}>
            ...
            >
              ...
              {sortOptionsData && sortOptionsData.length > 0 && sortOptionsData.map((option, oid) =>
                <SwiperSlide key={oid}>
                  ...
                  {sortBy === option.name && <Image src={option.selectedImage} alt={option.name} width="50" height="50" />}
                  {sortBy !== option.name && <Image src={option.image} alt={option.name} width="50" height="50" />}
      ...
      >
        <div className={loading ? " filtered-products loading-overlar" : "filtered-products" }>
              ...
              <Link href={`/product/${item.sku}`}>
                <div className="productsslide">
                  <Swiper
                    ...
                    modules={[Pagination]}
                  >
                    {item.media_gallery_entries && item.media_gallery_entries.length > 0 && item.media_gallery_entries.map((cat) => <SwiperSlide key={cat.id}><Image src={imgUrl + cat.file} alt={cat.label} width="150" height="225" /></SwiperSlide>)}
                  </Swiper>
                </div>
                </Link>
              </div>
              {item?.extension_attributes?.price && <div className="brandname"  style={{paddingLeft:"10px"}}><strong>{item?.extension_attributes?.brand_name}</strong></div>}
              <div className="fptitle text-muted p_short-desc" >
                {shortDesc(item.name)}
                
              </div>
              <div className="fprice" style={{display:"flex",paddingLeft:"10px"}}>
                <del>{item?.extension_attributes?.price}</del>
                <span className='text-primary' style={{marginLeft:"8px",fontSize:"14px"}}>{item?.extension_attributes?.product_discount_price}</span>
              </div>
            </div>
          })}
